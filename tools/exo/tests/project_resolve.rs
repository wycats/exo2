...
#[test]
fn project_snapshot_imports_available_projection_without_epochs_file() {
    let temp = tempfile::tempdir().expect("create tempdir");
    let root = temp.path();
    let home = temp.path().join("home");
    let config_home = temp.path().join("config");
    ...
    for entry in std::fs::read_dir(&projection_dir).expect("read projection directory") {
        let path = entry.expect("projection entry").path();
        if path.extension().and_then(|extension| extension.to_str()) == Some("sql") {
            std::fs::remove_file(path).expect("remove projection file");
        }
    }
    std::fs::write(
        projection_dir.join("axioms.sql"),
        "-- Auto-generated by exo. Regenerate: exo status\n",
    )
    .expect("write legacy projection table");
    std::fs::remove_file(&project_db).expect("delete project DB");
    ...
    assert!(
        project_db.exists(),
        "any canonical projection table should make cacheless state importable"
    );
}
