...
fn future_writer_errors_match_between_direct_and_ready_daemon(backend: &str) {
    ...
    let mut direct_error = direct["error"].clone();
    let direct_steering = direct_error
        .get_mut("details")
    ...
    assert_eq!(routed["error"], direct_error);
...
async fn linked_worktrees_share_one_workbench_host_with_workspace_scoped_sessions(backend: &str) {
    ...
    assert_eq!(
        primary_launch_retry, primary_launch,
        "a same-ID launch retry must return the exact live enrollment response"
    );
    ...
    let consumed_retry = send_machine_operation(
        &primary,
        "workbench-primary-launch",
        &["workbench", "launch"],
        serde_json::json!({}),
    ...
    .await;
    assert_eq!(consumed_retry["status"], "error", "{consumed_retry}");
    assert_eq!(
        consumed_retry["error"]["details"]["kind"], "workbench.launch_replay_unavailable",
        "{consumed_retry}"
    );
    assert_eq!(
        consumed_retry["error"]["details"]["retry_with_new_request_id"], true,
        "{consumed_retry}"
    );
    let fresh_launch = send_machine_operation(
        &primary,
        "workbench-primary-launch-after-consumption",
        &["workbench", "launch"],
        serde_json::json!({}),
    )
    .await;
    assert_eq!(fresh_launch["status"], "ok", "{fresh_launch}");
    assert_ne!(
        launch_parts(&fresh_launch).1,
        launch_parts(&primary_launch).1,
        "a deliberate new request ID must issue fresh enrollment authority"
    );
