<!-- exo:74 ulid:01kg5m2xyntcsgd5xzacbmmdxp -->
...
We have fixed the "API Acquired" bug (Phase 44) and established E2E infrastructure (Phase 45). Now we need to rigorously define _what_ the Dashboard does and verify it automatically to prevent regressions.
...
### 2b. Derived Root Invalidation

- **Given**: A derived root (e.g., `derived.phase-health`) is displayed in the Dashboard.
- **When**: A dependent file (`plan.toml` or `implementation-plan.toml`) is modified via the `exo` CLI.
- **Then**:
  - The derived root invalidates and recomputes.
  - The Dashboard UI refreshes to reflect the new derived values.
  - The Phase Details Tree View reflects the updated derived summary.
  - **Constraint**: The test MUST assert that invalidation happens via derived root refresh, not manual reload.
