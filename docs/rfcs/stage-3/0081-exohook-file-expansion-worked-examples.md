<!-- exo:81 ulid:01kmzxbcxyptxcfqntvwvm18k7 -->

# RFC 81: Exohook: File Expansion Worked Examples

- **Supersedes**: RFC 10015, RFC 10073, RFC 10016, RFC 10074, RFC 0168, RFC 10089
...
- pre-commit correctness is about the _index_ (what will be committed)
...
When `exohook validate` is invoked without a lane argument (or with an invalid lane), the CLI MUST display a helpful, colorful listing that:

1. **Explains what lanes are** (brief intro for first-time users)
2. **Lists all configured lanes** with their metadata from `hooks.toml`
3. **Shows git hook associations** (which lanes run automatically)
4. **Provides actionable quick-start commands**

Example output:

```
error: no lane specified

A "lane" is a predefined validation context—it determines which checks run
and which files they examine. Choose the lane that matches your workflow.

┌─ Available Validation Lanes ─────────────────────────────────────────────────┐
│                                                                              │
│  dev         Dev (uncommitted)                                               │
│              Quick feedback on your current changes before staging.          │
│              › 6 checks: check, lint, rust-fmt, rust-clippy, ...             │
│                                                                              │
│  coherence   pre-commit (staged)                            ⚡ git: pre-commit│
│              Runs automatically when you commit. Fix issues before pushing.  │
│              › 6 checks (parallel): check, lint, rust-fmt, ...               │
│                                                                              │
│  gate        pre-push (committed not pushed)                 ⚡ git: pre-push │
│              Runs tests before code leaves your machine. Usually ~30s.       │
│              › 2 checks (parallel): test, rust-coverage                      │
│                                                                              │
│  ci          CI (HEAD)                                                       │
│              Same as gate, but can run anytime. Mirror what CI will run.     │
│              › 2 checks: test, rust-coverage                                 │
│                                                                              │
└──────────────────────────────────────────────────────────────────────────────┘

💡 Quick start:
   exohook validate dev        # Check your current work
   exohook validate coherence  # Run what pre-commit runs
   exohook validate gate       # Run what pre-push runs
```

**Color scheme:**

- Lane IDs: **bold cyan** (what to type)
- Labels: **white**
- Descriptions: **dim white**
- Git hook badges: **yellow**
- Quick-start commands: **green**
- Box frame: **dim white**

This display is generated dynamically from `hooks.toml`, so project-specific lanes and descriptions appear automatically.
...
Rationale: the canonical lane IDs (`dev`, `coherence`, `gate`, `ci`) are meaningful to the system, but _not inherently meaningful_ to new contributors. Labels/descriptions allow the file to explain itself without changing the underlying model.
...
When a check consumes `paths`, `exohook` must define _exactly_ where and how the list appears.
...
Some checks are _autofixable_: they may modify files (formatters, or linters in `--fix` mode).
...
- Restage must _never_ stage unrelated working-tree changes.
...
3. Compute `changed_paths` = working-tree paths that changed _within_ `input_paths`.
4. If `changed_paths` is non-empty, run `git add -- <changed_paths>`.

**Containment enforcement**:

If an autofix tool modifies files _outside_ `input_paths`, `exohook` should treat that as a configuration error and fail with a recovery-oriented message. (This prevents “formatter ran on the whole repo” from silently staging unexpected changes.)
...
- In `coherence`: after formatting succeeds, `exohook` automatically restages _only the files from the lane list that actually changed_.
...
The following features from this RFC are **not yet implemented**:

- `{{files}}` runtime substitution (validation-only today)
- Fileset computation from git state
- Chunking semantics for long file lists
- `packages` input mode
- Watch mode
