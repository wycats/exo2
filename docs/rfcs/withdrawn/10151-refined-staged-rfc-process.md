<!-- exo:10151 ulid:01kmzxbczyf5e3pexqhgk8jgw4 -->
...
- **Artifacts**:
  ...
  - **Implementation Plan**: A `implementation-plan.toml` file generated and validated (RFC 0030).
...
- **Exit Criteria**:
  ...
  - **Axiom Check**: The plan explicitly aligns with `AGENTS.md` and `docs/manual/architecture/axioms.md`.
...
- **Artifacts**:
  ...
  - **Drift Check**: `task-list.md` matches `implementation-plan.toml` (RFC 0030).
...
- **Artifacts**:
  ...
  - **Manual Sync**: The feature is documented in `docs/manual/` (RFC 012).
  ...
  - **Axiom Update**: Any new principles are promoted to `docs/manual/architecture/axioms.md`.
...
- **Process**:
  ...
  3.  **Proposal**: The Agent builds a proposal for changing the **Project Plan** (`plan.toml` / `plan-outline.md`).
...
The `exo` CLI (RFC 0028) and the Workflow Automation (RFC 0030) are the **Enforcement Mechanisms** for this process.

- **RFC 0030 Relationship**:

  - **Stage 2 Gate**: RFC 0030's `exo phase prepare` is the tool that generates the `implementation-plan.toml` required for Stage 2.
...
- `exo rfc promote <id>`: Should prompt for confirmation and run specific checks for the target stage (e.g., "Does `implementation-plan.toml` exist?" for Stage 2).
