<!-- exo:162 ulid:01kg5kp2k1e0w7yp181jbtethm -->
...
**Relationship to RFC 0008 (Agent Context Links)**:
This RFC (Copilot Resources) focuses on _exposing_ resources to the AI in the chat interface (via `#` references). RFC 0008 focuses on _linking_ resources within the documents themselves (via `[Link](exo:...)`).

- **Convergence**: Both should use the same underlying URI scheme (`exo:...`) and resolution logic.
- **Synergy**: The `LanguageModelResourceProvider` can use the resolution logic defined in RFC 0008 to traverse the graph.
...
relations:
  ...
  - id: 0035
    type: enforced-by
    description: "The process defined here is enforced by the tooling in RFC 0024."
```
...
> "RFC 0030 (Stage 1). Implements RFC 0012. Enforced by RFC 0024. Status: Active."
