...
| 5 | Resolve remaining numeric ID collision `0060`. | It is the only remaining duplicate numeric ID and still makes ID-only RFC operations unsafe for that number. | Review prompt-patterns survivor, then repair the withdrawn dirty-tree steering tombstone behind RFC `0117`. |
